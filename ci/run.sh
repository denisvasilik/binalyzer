...
then
    ...
    sed -i 's/__tag__ = ""/__tag__ = "'0.0.10a${TRAVIS_BUILD_NUMBER}'"/g' binalyzer/__init__.py
    sed -i 's/__tag__ = ""/__tag__ = "'0.0.10a${TRAVIS_BUILD_NUMBER}'"/g' setup.py
fi
...
if [ ! -z "$TRAVIS_COMMIT" ]
then
    sed -i 's/__commit__ = \"0000000\"/__commit__ = \"'${TRAVIS_COMMIT::7}'\"/g' binalyzer/__init__.py
fi
